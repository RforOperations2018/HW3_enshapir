...
pal <- colorNumeric(c("green", "red"), 1:2)

factpal <- colorFactor(topo.colors(7), sandy.track$STROMTYPE)

#plot sandy track and color based on storm type
leaflet()%>% addTiles() %>%
  addPolylines(data = sandy.track, color = ~factpal(STORMTYPE), group = "Storm Track") %>% 
